...
class abrp_Blog_Authors extends WP_Widget {
    function abrp_Blog_Authors() {
		$widget_ops = array(  'description' => __('A widget that displays all authors based on the recent posts ', 'blog_authors_dsc') );
		$control_ops = array();
		$this->WP_Widget( 'abrp_blog_authors', __('Authors Based on Recent Posts', 'abrp_blog_authors'), $widget_ops, $control_ops );
		// wp_enqueue_style( 'abrp_PluginStylesheet' );
        // add plugin style
		...
		//add_action( 'profile_personal_options', 'abrp_show_extra_profile_fields' );
	} // end of abrp_Blog_Authors
